  ...
  const sendMessage = useCallback(
    async (text) => {
      ...
      try {
        ...
        // iOS Safari may have res.body === null or no getReader support
        const canStream =
          res.body != null && typeof res.body.getReader === "function";

        const processSSELines = (rawText) => {
        ...
        if (canStream) {
          ...
          if (buffer) processSSELines(buffer);
        } else {
          // Fallback for browsers without ReadableStream support (older iOS Safari)
          const rawText = await res.text();
          processSSELines(rawText);
        }
      } catch (err) {
