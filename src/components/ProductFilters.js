...
import { FiFilter, FiX, FiChevronDown, FiChevronRight, FiSearch } from "react-icons/fi";
...
export default function ProductFilters({ products, onFiltersChange, activeFilters, onClearFilters }) {
  ...
  useEffect(() => {
    ...
    setPriceRange(
      activeFilters.priceRange
        ? { min: activeFilters.priceRange.min || "", max: activeFilters.priceRange.max === Infinity ? "" : activeFilters.priceRange.max || "" }
        : { min: "", max: "" }
    );
  }, [activeFilters]);
  ...
  const handleSearch = (val) => {
    setSearchTerm(val);
    onFiltersChange({ ...activeFilters, searchTerm: val });
  };
    ...
    <div className="pf-card card">
      ...
      <div className="pf-header">
        <div className="pf-header-left">
          <div className="pf-header-icon"><FiFilter size={16} strokeWidth={2.5} /></div>
          <span className="pf-header-title">Filters</span>
        </div>
        {activeCount > 0 && (
          <div className="pf-header-right">
            <span className="pf-active-count">{activeCount} active</span>
            <button className="pf-clear-btn" onClick={onClearFilters} aria-label="Clear all filters">
      ...
      <div className="pf-body card-body">
        ...
        <div className="pf-section">
          <label className="pf-section-label">Search</label>
          <div style={{ position: "relative" }}>
            <FiSearch size={14} style={{ position: "absolute", left: "0.75rem", top: "50%", transform: "translateY(-50%)", color: "var(--text-muted)", pointerEvents: "none" }} />
            <input
              type="text"
              className="pf-input"
        ...
        <div className="pf-section">
          ...
          <select className="pf-select form-select" value={sortBy} onChange={(e) => handleSort(e.target.value)}>
        ...
        <div className="pf-section">
          ...
          {categoriesLoading ? (
            ...
            <div className="pf-state pf-state--error">Failed to load categories</div>
          ) : categories?.length > 0 ? (
            <div className="pf-categories">
              {categories.map((cat) => {
                const hasSubs  = cat.subcategories?.length > 0;
                const isOpen   = expandedCategories.has(cat._id);
                const checked  = activeFilters.categories?.includes(cat.name) || false;
                return (
                  <div key={cat._id} className="pf-cat-item">
                    <div className="pf-cat-row">
                      {hasSubs ? (
                        <button className="pf-expand-btn" onClick={() => toggleExpand(cat._id)} aria-label={isOpen ? "Collapse" : "Expand"}>
                      ...
                      />
                      <label className="pf-cat-label" htmlFor={`cat-${cat._id}`}>{cat.name}</label>
                    </div>
                        ...
                        {cat.subcategories.filter((s) => s.isActive).map((sub) => (
                          <div key={sub._id} className="pf-subcat-row">
                            <input
                              ...
                              className="pf-checkbox"
                              id={`sub-${sub._id}`}
                              checked={activeFilters.categories?.includes(sub.name) || false}
                              onChange={(e) => handleCategoryChange(sub.name, e.target.checked)}
                            />
                            <label className="pf-subcat-label" htmlFor={`sub-${sub._id}`}>{sub.name}</label>
                          </div>
                        ))}
                      </div>
                    )}
            ...
            <div className="pf-state pf-state--empty">No categories available</div>
          )}
        </div>
        ...
        <div className="pf-section">
          ...
          {prices.length > 0 && (
            <div className="pf-price-hint">₹{minPrice.toLocaleString()} – ₹{maxPrice.toLocaleString()}</div>
          )}
          ...
          <div className="pf-active-section">
            ...
            <div className="pf-chips">
              {activeFilters.categories?.map((c) => (
                <span key={c} className="pf-chip pf-chip--cat">{c}</span>
              ))}
              {activeFilters.priceRange && (
              ...
              {activeFilters.searchTerm && (
                <span className="pf-chip pf-chip--search">"{activeFilters.searchTerm}"</span>
              )}
              {activeFilters.sortBy && (
                <span className="pf-chip pf-chip--sort">
                  {activeFilters.sortBy.replace(/-/g, " ").replace(/\b\w/g, (l) => l.toUpperCase())}
                </span>
              )}
            </div>
          </div>
