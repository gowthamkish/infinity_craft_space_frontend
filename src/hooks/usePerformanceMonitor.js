...
export const usePerformanceMonitor = (
  ...
  useEffect(() => {
    ...
    if (renderTime > 16) {
      ...
      console.warn(
        `[Performance] ${componentName} render took ${renderTime}ms (render #${renderCount.current})`,
      );
    }
  ...
  const measureAsync = useCallback(
    async (operationName, asyncOperation) => {
      ...
      try {
        ...
        console.log(
          `[Performance] ${componentName}.${operationName} took ${end - start}ms`,
        );
        return result;
      } catch (error) {
        const end = performance.now();
        console.error(
          `[Performance] ${componentName}.${operationName} failed after ${end - start}ms`,
          error,
        );
        throw error;
      }
  ...
  const measure = useCallback(
    (operationName, operation) => {
      ...
      try {
        ...
        const end = performance.now();
        if (end - start > 5) {
          // Log operations taking more than 5ms
          console.log(
            `[Performance] ${componentName}.${operationName} took ${end - start}ms`,
          );
        }
...
export const useMemoryMonitor = (
  ...
  useEffect(() => {
    ...
    const logMemoryUsage = () => {
      ...
      console.log(
        `[Memory] ${componentName} - Used: ${usedMB}MB, Total: ${totalMB}MB, Limit: ${limitMB}MB`,
      );

      // Warn if memory usage is high
      if (usedMB > limitMB * 0.9) {
        console.warn(
          `[Memory] High memory usage detected in ${componentName}!`,
        );
      }
    };
